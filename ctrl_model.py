...
class CTRL_Model(object):
    ...
    def cross_modal_comb(self, visual_feat, sentence_embed, batch_size):
        ...
        concat_feature = tf.reshape(tf.concat(2, [vv_feature, ss_feature]),
        ...
        comb_feature = tf.reshape(tf.concat(2, [mul_feature, add_feature, concat_feature]),
    ...
    def visual_semantic_infer(self, visual_feature_train, sentence_embed_train, visual_feature_test,
                              sentence_embed_test):
        name = "CTRL_Model"
        with tf.variable_scope(name):
            ...
            cross_modal_vec_train = self.cross_modal_comb(transformed_clip_train_norm, transformed_sentence_train_norm,
                                                          self.batch_size)
            sim_score_mat_train = vs_multilayer.vs_multilayer(cross_modal_vec_train, "vs_multilayer_lt",
            ...
            cross_modal_vec_test = self.cross_modal_comb(transformed_clip_test_norm, transformed_sentence_test_norm,
                                                         self.test_batch_size)
            sim_score_mat_test = vs_multilayer.vs_multilayer(cross_modal_vec_test, "vs_multilayer_lt", reuse=True,
                                                             middle_layer_dim=1000)
    ...
    def construct_model(self):
        ...
        sim_reg_mat, sim_reg_mat_test = self.visual_semantic_infer(self.visual_featmap_ph_train, self.sentence_ph_train,
