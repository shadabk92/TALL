...
def do_eval_slidingclips(sess, vs_eval_op, model, movie_length_info, iter_step, test_result_output):
    ...
    for k in range(len(IoU_thresh)):
        ...
        test_result_output.write("Step " + str(iter_step) + ": IoU=" + str(IoU_thresh[k]) + ", R@10: " + str(
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        for step in xrange(max_steps):
            ...
            _, loss_value, offset_pred_v, loss_reg_v = sess.run([vs_train_op, loss_align_reg, offset_pred, loss_reg],
                                                                feed_dict=feed_dict)
